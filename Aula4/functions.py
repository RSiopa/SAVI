...
class BoundingBox:
    ...
    def __init__(self, x1, y1, w, h, area):
        self.x1 = x1
        self.y1 = y1
        self.w = w
        self.h = h
...
class Detection(BoundingBox):
    ...
    def draw(self, image_gui, color=(255, 0, 0)):
        ...
        image = cv2.putText(image_gui, 'D' + str(self.id), (self.x1, self.y1 - 5), cv2.FONT_HERSHEY_SIM, 1, color, 2, cv2.LINE_AA)
...
class Tracker():
    ...
    def draw(self, image_gui, color=(255, 0, 255)):
        last_detection = self.detection[-1]

        cv2.rectangle(image_gui, (last_detection.x1, last_detection.y1), (last_detection.x2, last_detection.y2), color, 3)

        image = cv2.putText(image_gui, 'T' + str(self.id), (last_detection.x1 - 40, last_detection.y1 - 5), cv2.FONT_HERSHEY_SIM, 1, color, 2, cv2.LINE_AA)
